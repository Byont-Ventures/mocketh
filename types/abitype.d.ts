/** @see https://github.com/wagmi-dev/abitype/blob/main/src/examples/types.ts */
...
> = TAbiFunction['inputs'] extends infer TInputs extends readonly AbiParameter[]
  ? // Check if valid ABI. If `TInputs` is `never` or `TAbi` does not have the same shape as `Abi`, then return optional `readonly unknown[]` args.
    ...
    ? {
         ...
         * on {@link abi} for type inference.
         */
...
/**
 * Contract configuration object for inferring function name and arguments based
 * on {@link TAbi}.
 */
 ...
 *
 * @param TContract - Contract config in `{ abi: Abi, functionName: string }`
 *   format
...
/**
 * Converts array of {@link AbiEvent} parameters to corresponding TypeScript
 * primitive types.
 *
 * @param TAbiEventParameters - Array of {@link AbiEvent} parameters to convert
 *   to TypeScript representations
